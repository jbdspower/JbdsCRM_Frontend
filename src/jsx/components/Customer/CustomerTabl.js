...
const CustomerListTbl = ({loading, setLoading}) => {
  ...
  const handleDeleteUser = (userId) => {
    axios
      .delete(`${api.api}customer/${userId}`) // DELETE request for permanent deletion
      .then(() => {
        fetchAllProducts();
      })
      .catch((err) => {
  ...
  const handleToggleStatus = (user) => {
    ...
    axios
      .patch(`${api.api}customer/${user._id}`, { status: newStatus })
      .then(() => {
        fetchAllProducts();
      })
      .catch((err) => {
  ...
  const handleSaveChanges = () => {
    axios
      .patch(`${api.api}customer/${currentUser._id}`, updatedUser)
      .then(() => {
        fetchAllProducts();
        ...
        <div className="card-body p-0">
          <div className="table-responsive active-projects style-1 ItemsCheckboxSec shorting">
            {/* <div className="tbl-caption">
                            <h4 className="heading mb-0">Customer List</h4>
                            <div>
            ...
            <div id="user-tbl_wrapper" className="dataTables_wrapper no-footer">
              ...
              <div className="d-sm-flex text-center justify-content-between align-items-center">
                ...
                <div className="dataTables_paginate paging_simple_numbers">
                  ...
                  <span>
                    {Array(Math.ceil(allUser.length / sort))
                      .fill()
                      .map((_, i) => (
                        <Link
                          key={i}
                  ...
                  <Link
                    className={`paginate_button next ${
                      currentPage + 1 >= Math.ceil(allUser.length / sort)
                        ? "disabled"
                        : ""
                    }`}
                    to="#"
...
export default CustomerListTbl;
