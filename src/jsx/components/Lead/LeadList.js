...
const LeadListPage = (fromDate, toDate) => {
  ...
  const filterThroughDate = () => {
    if (!fromDate || !toDate) {
      swal("Please select both From and To dates.");
      return;
    }

    const filtered = list.filter((lead) => {
      const leadDate = new Date(lead.date); // Adjust 'date' to match API field name
      return leadDate >= new Date(fromDate) && leadDate <= new Date(toDate);
    });

    setFilteredLeads(filtered);
    setDateName(`Filtered from ${fromDate} to ${toDate}`);
  };
  ...
  const getLeadList = () => {
    axiosInstance.get("getAllLead")
      .then((res) => {
        console.log("API Response:", res.data); // Debugging: Check the API response
        if (Array.isArray(res.data)) {
          setList(res.data);
          setFilteredLeads(res.data); // Initially show all leads
        } else {
          console.error("Expected an array but got:", res.data);
          setList([]); // Fallback to an empty array
          setFilteredLeads([]);
        }
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    const newFilteredResults = filteredLeads.filter((e) => {
      const itemDate = new Date(e.createdAt);
      const itemYear = itemDate.getFullYear();
      const itemMonth = itemDate.getMonth();
      const formattedItemDate = itemDate.toISOString().split("T")[0]; // Extract only date part
      ...
      if (filterDate === "This Month") {
        ...
        const startDate = new Date(currentYear, currentMonth, 20); // 20th of current month
        const endDate = new Date(currentYear, currentMonth + 1, 21); // 21st of next month
      
        // Convert item date to a comparable format
        const itemDate = new Date(itemDate.getFullYear(), itemDate.getMonth() - 1, itemDate.getDay()); // Assuming item.month is 1-based
      ...
      if (filterDate === "Last Month") {
        const lastMonth = new Date(today.setMonth(currentMonth - 1));
        return itemYear === lastMonth.getFullYear() && itemMonth === lastMonth.getMonth();
      } 
      ...
      return true; // Default return all leads
    });
  
    setFilteredLeads(newFilteredResults);
  }, [filterDate]);
   // Ensure `filteredLeads` is included in dependencies
  
  
  const handleCardClick = (index) => {
    setSelectedCard(index);
    const filteredData = leadCounters[index].filter(); // Apply the filter
    setFilteredLeads(filteredData);
    console.log("ajsddas",filteredData)
  };
  ...
  return (
    <>
    <LeadNavHeader getLeadList={getLeadList} IsMenu={true} mainTitle="Lead Management" pageTitle="Leads" parentTitle="Lead Management" setfilterDate={setfilterDate} filterDate={filterDate}/>
    <QuaotationForm show={showQuotForm} setShow={setQuotForm} />

    <div style={{ padding: "10px" }}>
      {console.log("lead counter",leadCounters)}
    <Row gutter={[0, 0]} style={{ marginBottom: "10px", padding: "2px"}}>
          ...
          {console.log("filter leads",filteredLeads)}
